...
// `signal` ends a write the reader walked away from: without it, a closed connection
// never drains, and the writer would wait on it for the life of the process.
export async function writeZip(out, entries, when = new Date(), signal = undefined) {
  const { time, date } = dosTime(when);
  const central = [];
  let offset = 0;
  const write = async (buf) => {
    offset += buf.length;
    if (!out.write(buf)) await once(out, 'drain', { signal });
  };
